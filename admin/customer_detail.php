...
try {
    ...
    if ($activePlan) {
        try {
            ...
            $stmt = $conn->prepare("SELECT id, session_number, service_name, focus_notes, before_image_path, after_image_path, completed_at FROM treatment_plan_sessions WHERE plan_id = ? ORDER BY session_number");
            $stmt->execute([$activePlan['id']]);
            $planSessions = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<div class="space-y-6">
                                    ...
                                    <article class="rounded-[1.5rem] border p-5 transition hover:-translate-y-0.5 hover:shadow-sm <?= $cardClass ?>">
                                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                            <div class="min-w-0">
                                                ...
                                                <?php if (!empty($session['before_image_path']) || !empty($session['after_image_path'])): ?>
                                                    <div class="mt-3 flex flex-wrap items-center gap-3">
                                                        <?php if (!empty($session['before_image_path'])): ?>
                                                            <a
                                                                href="../<?= htmlspecialchars($session['before_image_path']) ?>"
                                                                target="_blank"
                                                                class="inline-flex items-center gap-2 text-xs font-black text-amber-600 hover:underline"
                                                            >
                                                                <img
                                                                    src="../<?= htmlspecialchars($session['before_image_path']) ?>"
                                                                    alt="Before image"
                                                                    class="h-9 w-9 rounded-xl border border-amber-200 object-cover"
                                                                />
                                                                <span class="bg-amber-50 px-2 py-1 rounded-lg">Before</span>
                                                            </a>
                                                        <?php endif; ?>
                                                        <?php if (!empty($session['after_image_path'])): ?>
                                                            <a
                                                                href="../<?= htmlspecialchars($session['after_image_path']) ?>"
                                                                target="_blank"
                                                                class="inline-flex items-center gap-2 text-xs font-black text-emerald-600 hover:underline"
                                                            >
                                                                <img
                                                                    src="../<?= htmlspecialchars($session['after_image_path']) ?>"
                                                                    alt="After image"
                                                                    class="h-9 w-9 rounded-xl border border-emerald-200 object-cover"
                                                                />
                                                                <span class="bg-emerald-50 px-2 py-1 rounded-lg">After</span>
                                                            </a>
                                                        <?php endif; ?>
                                                    </div>
                                                <?php endif; ?>
    ...
    <?php if (!empty($completedPlans)): ?>
        <div class="mt-12">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <?php 
                // Get sessions for completed plans
                    ...
                    try {
                        $stmt = $conn->prepare("
                            SELECT tps.*, 
                                   ...
                                   sr.clinical_observations, sr.outcome_rating
                            FROM treatment_plan_sessions tps
                            LEFT JOIN treatment_session_schedules tss ON tps.id = tss.plan_session_id
                        ...
                        ");
                        $stmt->execute([$p['id']]);
                        $completedPlanSessions = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    } catch (Throwable $e) { }
                    
                    $completedSessionCount = count(array_filter($completedPlanSessions, fn($s) => !empty($s['completed_at'])));
                    $totalSessionCount = count($completedPlanSessions);
                ?>
                    <div class="rounded-2xl border border-slate-200 bg-white p-5 transition hover:bg-slate-50 hover:shadow-sm cursor-pointer" onclick="openCompletedPlanModal(<?= (int)$p['id'] ?>)">
    ...
    async function openCompletedPlanModal(planId) {
        try {
            const response = await fetch(`../backend/api/get_completed_plan.php?plan_id=${planId}`);
            const result = await response.json();
            ...
            if (result.success) {
                ...
                // Header info
                document.getElementById('completedPlanTitle').textContent = plan.title;
                
                // Build header section
                let headerHtml = `
                    <div class="bg-gradient-to-r from-sky-50 to-cyan-50 rounded-2xl p-6 mb-6">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div>
                                <p class="text-xs font-black uppercase tracking-wider text-slate-500 mb-1">Bắt đầu</p>
                                <p class="font-bold text-slate-800">${plan.start_date ? new Date(plan.start_date).toLocaleDateString('vi-VN') : 'N/A'}</p>
                            </div>
                            <div>
                                <p class="text-xs font-black uppercase tracking-wider text-slate-500 mb-1">Hoàn thành</p>
                                <p class="font-bold text-slate-800">${plan.completed_date ? new Date(plan.completed_date).toLocaleDateString('vi-VN') : 'N/A'}</p>
                            </div>
                            <div>
                                <p class="text-xs font-black uppercase tracking-wider text-slate-500 mb-1">Bác sĩ</p>
                                <p class="font-bold text-slate-800">${plan.specialist_name || 'N/A'}</p>
                            </div>
                            <div>
                                <p class="text-xs font-black uppercase tracking-wider text-slate-500 mb-1">Số buổi</p>
                                <p class="font-bold text-slate-800">${plan.sessions.length} buổi</p>
                            </div>
                        </div>
                        ${plan.overall_goal ? `
                            <div class="mt-4 pt-4 border-t border-sky-200">
                                <p class="text-xs font-black uppercase tracking-wider text-slate-500 mb-1">Mục tiêu điều trị</p>
                                <p class="text-sm text-slate-700">${plan.overall_goal}</p>
                            </div>
                        ` : ''}
                    </div>
                `;
                ...
                plan.sessions.forEach((session, index) => {
                    const hasBeforeImage = session.before_image_path;
                    const hasAfterImage = session.after_image_path;
                    const hasAnyImage = hasBeforeImage || hasAfterImage;
                    const hasClinicalNotes = session.clinical_observations || session.skin_response || session.products_used || session.home_care || session.next_focus;
                    ...
                    sessionsHtml += `
                        <div class="rounded-2xl border border-slate-200 overflow-hidden bg-white shadow-sm">
                            ...
                            <div class="bg-slate-800 px-5 py-4 flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <span class="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-500 text-white text-sm font-black">
                                        <span class="material-symbols-outlined text-[16px]">check</span>
                                    </span>
                                    <div>
                                        <p class="font-black text-white">Buổi #${session.session_number}</p>
                                        <p class="text-xs text-slate-400">${session.service_name}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-slate-400">Hoàn thành</p>
                                    <p class="text-sm font-bold text-white">${session.completed_at ? new Date(session.completed_at).toLocaleDateString('vi-VN') : 'N/A'}</p>
                                </div>
                            </div>
                            ...
                            <div class="p-5">
                                ...
                                ${session.focus_notes ? `
                                    <div class="mb-4">
                                        <p class="text-xs font-black uppercase tracking-wider text-slate-500 mb-1">Trọng tâm buổi điều trị</p>
                                        <p class="text-sm text-slate-700 bg-sky-50 rounded-xl px-4 py-3">${session.focus_notes}</p>
                                    </div>
                                ` : ''}
                                
                                <!-- Before/After Images - Full Width -->
                                ${hasAnyImage ? `
                                    <div class="mb-4">
                                        <p class="text-xs font-black uppercase tracking-wider text-slate-500 mb-3">Hình ảnh trước & sau điều trị</p>
                                        <div class="grid grid-cols-2 gap-4">
                                            ${hasBeforeImage ? `
                                                <div class="relative group">
                                                    <a href="../${session.before_image_path}" target="_blank" class="block">
                                                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all rounded-2xl flex items-center justify-center">
                                                            <span class="material-symbols-outlined text-white text-4xl opacity-0 group-hover:opacity-100 transition-opacity">open_in_full</span>
                                                        </div>
                                                        <img src="../${session.before_image_path}" alt="Before" class="w-full h-48 object-cover rounded-2xl border-2 border-amber-200">
                                                    </a>
                                                    <div class="absolute top-2 left-2 bg-amber-500 text-white text-xs font-black px-3 py-1 rounded-full">
                                                        TRƯỚC
                                                    </div>
                                                </div>
                                            ` : '<div class="bg-slate-100 rounded-2xl h-48 flex items-center justify-center"><p class="text-slate-400 text-sm">Chưa có ảnh trước</p></div>'}
                                            ${hasAfterImage ? `
                                                <div class="relative group">
                                                    <a href="../${session.after_image_path}" target="_blank" class="block">
                                                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all rounded-2xl flex items-center justify-center">
                                                            <span class="material-symbols-outlined text-white text-4xl opacity-0 group-hover:opacity-100 transition-opacity">open_in_full</span>
                                                        </div>
                                                        <img src="../${session.after_image_path}" alt="After" class="w-full h-48 object-cover rounded-2xl border-2 border-emerald-200">
                                                    </a>
                                                    <div class="absolute top-2 left-2 bg-emerald-500 text-white text-xs font-black px-3 py-1 rounded-full">
                                                        SAU
                                                    </div>
                                                </div>
                                            ` : '<div class="bg-slate-100 rounded-2xl h-48 flex items-center justify-center"><p class="text-slate-400 text-sm">Chưa có ảnh sau</p></div>'}
                                        </div>
                                    </div>
                                ` : ''}
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        ${session.clinical_observations ? `
                                            <div class="bg-violet-50 rounded-xl p-4">
                                                <p class="text-xs font-black uppercase tracking-wider text-violet-600 mb-2">Quan sát lâm sàng</p>
                                                <p class="text-sm text-slate-700">${session.clinical_observations}</p>
                                            </div>
                                        ` : ''}
                                        ${session.skin_response ? `
                                            <div class="bg-cyan-50 rounded-xl p-4">
                                                <p class="text-xs font-black uppercase tracking-wider text-cyan-600 mb-2">Phản ứng da</p>
                                                <p class="text-sm text-slate-700">${session.skin_response}</p>
                                            </div>
                                        ` : ''}
                                        ${session.pain_level !== null && session.pain_level !== '' ? `
                                            <div class="bg-rose-50 rounded-xl p-4">
                                                <p class="text-xs font-black uppercase tracking-wider text-rose-600 mb-2">Mức độ đau</p>
                                                <div class="flex items-center gap-2">
                                                    <div class="flex-1 bg-rose-200 rounded-full h-2">
                                                        <div class="bg-rose-500 h-2 rounded-full" style="width: ${(session.pain_level / 10) * 100}%"></div>
                                                    </div>
                                                    <span class="text-sm font-black text-rose-700">${session.pain_level}/10</span>
                                                </div>
                                            </div>
                                        ` : ''}
                                        ${session.outcome_rating ? `
                                            <div class="bg-amber-50 rounded-xl p-4">
                                                <p class="text-xs font-black uppercase tracking-wider text-amber-600 mb-2">Đánh giá kết quả</p>
                                                <div class="flex items-center gap-1">
                                                    ${[1,2,3,4,5].map(n => `
                                                        <span class="material-symbols-outlined text-lg ${n <= session.outcome_rating ? 'text-amber-500' : 'text-slate-300'}">star</span>
                                                    `).join('')}
                                                    <span class="ml-2 text-sm font-black text-amber-700">${session.outcome_rating}/5</span>
                                                </div>
                                            </div>
                                        ` : ''}
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ${session.products_used ? `
                                            <div class="bg-purple-50 rounded-xl p-4">
                                                <p class="text-xs font-black uppercase tracking-wider text-purple-600 mb-2">Sản phẩm sử dụng</p>
                                                <p class="text-sm text-slate-700">${session.products_used}</p>
                                            </div>
                                        ` : ''}
                                        ${session.home_care ? `
                                            <div class="bg-teal-50 rounded-xl p-4">
                                                <p class="text-xs font-black uppercase tracking-wider text-teal-600 mb-2">Chăm sóc tại nhà</p>
                                                <p class="text-sm text-slate-700">${session.home_care}</p>
                                            </div>
                                        ` : ''}
                                    </div>
                                    
                                    ${session.next_focus ? `
                                        <div class="mt-4 bg-gradient-to-r from-sky-50 to-cyan-50 rounded-xl p-4 border border-sky-200">
                                            <p class="text-xs font-black uppercase tracking-wider text-sky-600 mb-2">Trọng tâm cho buổi tiếp theo</p>
                                            <p class="text-sm text-slate-700 font-medium">${session.next_focus}</p>
                                        </div>
                                    ` : ''}
                                ` : ''}
                            </div>
                ...
                document.getElementById('completedPlanContent').innerHTML = headerHtml + sessionsHtml;
                document.getElementById('completedPlanModal').classList.remove('hidden');
            }
        } catch (error) {
    ...
    function closeCompletedPlanModal() {
        document.getElementById('completedPlanModal').classList.add('hidden');
    }
    </script>

    <!-- Completed Plan Modal -->
    <div id="completedPlanModal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" onclick="closeCompletedPlanModal()"></div>
        <div class="absolute left-1/2 top-1/2 w-full max-w-5xl max-h-[95vh] overflow-y-auto -translate-x-1/2 -translate-y-1/2">
            <div class="bg-white rounded-3xl shadow-2xl m-4">
                <div class="sticky top-0 bg-white rounded-t-3xl border-b border-slate-200 px-8 py-6 flex items-center justify-between z-10">
                    <div>
                        <h3 id="completedPlanTitle" class="text-2xl font-black text-slate-900">Completed Treatment Plan</h3>
                        <p id="completedPlanSubtitle" class="text-sm font-semibold text-slate-500 mt-1"></p>
                    </div>
                    <button onclick="closeCompletedPlanModal()" class="p-2 hover:bg-slate-100 rounded-full transition">
                        <span class="material-symbols-outlined text-slate-500">close</span>
                    </button>
                </div>
                <div id="completedPlanContent" class="p-8">
