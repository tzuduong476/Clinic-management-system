    ...
    if ($title === '' || $specialistId <= 0) {
        ...
        try {
            ...
            foreach ($sessions as $session) {
                ...
                if (
                    isset($_FILES["before_image_$sessionNumber"]) &&
                    is_array($_FILES["before_image_$sessionNumber"]) &&
                    ($_FILES["before_image_$sessionNumber"]['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK
                ) {
                    $tmp = $_FILES["before_image_$sessionNumber"]['tmp_name'];
                    $original = $_FILES["before_image_$sessionNumber"]['name'] ?? '';
                    ...
                    if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'], true)) {
                        ...
                        $filename = 'plan_' . $planId . '_session_' . $sessionNumber . '_before_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
                ...
                if (
                    isset($_FILES["after_image_$sessionNumber"]) &&
                    is_array($_FILES["after_image_$sessionNumber"]) &&
                    ($_FILES["after_image_$sessionNumber"]['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK
                ) {
                    $tmp = $_FILES["after_image_$sessionNumber"]['tmp_name'];
                    $original = $_FILES["after_image_$sessionNumber"]['name'] ?? '';
                    $ext = strtolower(pathinfo($original, PATHINFO_EXTENSION));
                    ...
                    if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'], true)) {
                        ...
                        $filename = 'plan_' . $planId . '_session_' . $sessionNumber . '_after_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
                        $target = $uploadDir . '/' . $filename;
                ...
                $stmt = $conn->prepare("
                    ...
                    SET service_id = ?, service_name = ?, focus_notes = ?, before_image_path = ?, after_image_path = ?
                    WHERE id = ?
                ");
                $stmt->execute([$serviceId, $serviceName, $focus, $beforeImagePath, $afterImagePath, $sessionId]);
            }
                                            ...
                                            <div class="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-2">
                                                <div>
                                                    ...
                                                    <?php if (!empty($session['before_image_path'])): ?>
                                                        <div class="mb-2">
                                                            <img src="../<?= h($session['before_image_path']) ?>" alt="Before" class="h-16 w-full rounded-xl object-cover border border-amber-200">
                                                        </div>
                                                    <?php endif; ?>
                                                    <input
                                                        type="file"
                                                        name="before_image_<?= h($number) ?>"
                                                        accept=".jpg,.jpeg,.png,.webp,image/*"
                                                        class="block h-11 w-full rounded-2xl border border-slate-200 bg-white px-2 py-2 text-[11px] font-semibold text-slate-500 file:mr-2 file:rounded-full file:border-0 file:bg-amber-50 file:px-2 file:py-1 file:text-[10px] file:font-black file:text-amber-600"
                                                    />
                                                </div>
                                                <div>
                                                    <label class="mb-2 block text-xs font-black uppercase tracking-[0.16em] text-slate-400">
                                                    ...
                                                    <?php if (!empty($session['after_image_path'])): ?>
                                                        <div class="mb-2">
                                                            <img src="../<?= h($session['after_image_path']) ?>" alt="After" class="h-16 w-full rounded-xl object-cover border border-emerald-200">
                                                        </div>
                                                    <?php endif; ?>
                                                    <input
                                                        type="file"
                                                        name="after_image_<?= h($number) ?>"
                                                        accept=".jpg,.jpeg,.png,.webp,image/*"
