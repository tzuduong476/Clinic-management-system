...
$specialistId = (int)($_POST['specialist_id'] ?? ($_GET['specialist_id'] ?? 0));
...
function ensureSessionImageColumn(PDO $conn): void
{
    try {
        $exists = $conn
            ->query("SHOW COLUMNS FROM treatment_plan_sessions LIKE 'session_image_path'")
            ->fetch(PDO::FETCH_ASSOC);

        if (!$exists) {
            $conn->exec("ALTER TABLE treatment_plan_sessions ADD COLUMN session_image_path VARCHAR(500) DEFAULT NULL AFTER focus_notes");
        }
    } catch (Throwable $e) {
        // Keep page usable when schema changes are restricted.
    }
}

ensureSessionImageColumn($conn);
        ...
        try {
            ...
            if ($activePlanId > 0) {
                ...
                foreach ($selectedSessions as $sessionData) {
                    ...
                    if (
                        isset($_FILES["session_image_$i"]) &&
                        is_array($_FILES["session_image_$i"]) &&
                        ($_FILES["session_image_$i"]['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK
                    ) {
                        $tmp = $_FILES["session_image_$i"]['tmp_name'];
                        $original = $_FILES["session_image_$i"]['name'] ?? '';
                        ...
                        if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'], true)) {
                            ...
                            $filename = 'plan_' . $planId . '_session_' . $i . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
                            $target = $uploadDir . '/' . $filename;
    
                            if (@move_uploaded_file($tmp, $target)) {
                                $sessionImagePath = 'uploads/treatment_sessions/' . $filename;
                            }
                        }
                    }
